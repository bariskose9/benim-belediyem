...
| 14 | **Docker imajı yerel makinede doğrulanamadı** | Docker Desktop derleme sırasında yanıt vermez hale geldi. Bunun yerine CI'a `docker` işi eklendi: imaj her PR'da GitHub Actions'ta derleniyor. Yine de imaj **çalıştırılarak** (kap ayağa kalkıyor mu) denenmedi | Docker yerelde tekrar çalıştığında bir kez `docker run` ile denenmeli |
